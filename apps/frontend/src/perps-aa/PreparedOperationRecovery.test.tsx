...
describe('visible preparation recovery', () => {
  ...
  it('backs off errors for 60 seconds and stops polling resolved status', async () => {
    ...
    await act(async () => {})
    expect(screen.queryByRole('button', { name: 'Resume deposit margin' })).not.toBeInTheDocument()
  ...
  it('does not leave account-confirmation resume disabled by a superseded status request', async () => {
    let finishStatus!: (result: typeof status) => void
    const getPreparationStatus = vi.fn(() => new Promise<typeof status>(resolve => { finishStatus = resolve }))
    const gate = createDeploymentConfirmationGate(async () => true, () => Date.now())
    const runtime = { smartAccount: { getPreparationStatus }, deploymentConfirmation: gate } as unknown as PerpsAaSmartAccountRuntime
    const saved = { ...operation, status: 'preparation-pending' as const, preparedOperation: undefined, reason: 'UNKNOWN' }
    const view = render(<PerpsAaRuntimeContext value={runtime}><PreparedOperationRecovery operation={saved} /></PerpsAaRuntimeContext>)
    await act(async () => {})
    expect(screen.getByRole('button', { name: 'Checking recovery…' })).toBeDisabled()
    await act(async () => {
      view.rerender(<PerpsAaRuntimeContext value={runtime}><PreparedOperationRecovery operation={{ ...saved, reason: 'ACCOUNT_DEPLOYMENT_PENDING' }} /></PerpsAaRuntimeContext>)
    })
    expect(screen.getByRole('button', { name: 'Resume deposit margin' })).toBeEnabled()
    await act(async () => { finishStatus({ ...status, recoverable: false }) })
    expect(screen.getByRole('button', { name: 'Resume deposit margin' })).toBeEnabled()
    expect(resume).not.toHaveBeenCalled()
    view.unmount()
  })
  ...
  it('offers owner verification for a signed attempt even when the status error has no reason', async () => {
    ...
    expect(screen.getByText('We couldn’t confirm this transaction')).toBeInTheDocument()
  ...
  it('guides an expired signed attempt through verification before offering status checks', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Wallet verified' })).toBeDisabled()
    expect(screen.getByText('2. Check transaction outcome')).toBeVisible()
    expect(screen.getByText('We couldn’t confirm this transaction')).toBeVisible()
    ...
    expect(screen.getByText('1. Verify your wallet')).toBeVisible()
    expect(screen.queryByRole('button', { name: 'Check recovery again' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Wallet verified' })).not.toBeInTheDocument()
