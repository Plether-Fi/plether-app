...
spec = do
  ...
  describe "USDC transfer parsing" $ do
    it "accepts only the exact canonical ERC-20 Transfer ABI" $ do
...
fixtureEmitterForTopic topic
  ...
  , Just lifecycleBook <- paOrderLifecycleBook defaultPerpsAddresses =
      lifecycleBook
