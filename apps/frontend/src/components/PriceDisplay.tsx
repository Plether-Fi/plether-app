  ...
  const getStatusStyles = (s: ProtocolStatus) => {
    switch (s) {
      ...
      case 'Paused':
        return 'bg-warning-bg text-warning border-warning/30'
      case 'Settled':
        return 'bg-brand-peach/20 text-brand-peach border-brand-peach/30'
    }
  }
    ...
    <div className="bg-surface-panel  border border-brand-border/30 p-4">
      ...
      {isLoading ? (
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-3xl font-bold text-content-secondary">--</span>
        </div>
      ) : (
        <div className="flex items-baseline gap-2">
          <TokenAmount amount={price.toFixed(2)} className="text-3xl font-bold text-content-primary" />
        </div>
      )}
